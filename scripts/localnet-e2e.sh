...
#
#   # Skip expensive subnet creation on re-runs:
#   EXISTING_NETUIDS=4,5,6 ./scripts/localnet-e2e.sh
# ============================================================================
...
# 3:2:1 stake skew forces _findBestMove to fire during processDeposit
# and _sortHotkeysByBalanceDesc to drain in a non-trivial order during withdraw.
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 1: Create 3 subnets + start emissions
# ═════════════════════════════════════════════════════════════════════════════

if [[ -n "${EXISTING_NETUIDS:-}" ]]; then
    log "Phase 1: Using existing subnets"
    IFS=',' read -ra NETUIDS <<< "$EXISTING_NETUIDS"
    for NETUID in "${NETUIDS[@]}"; do ok "netuid $NETUID (existing)"; done
else
...
# Force max_regs_per_block even on existing subnets (default 1 would rate-limit 3 registrations).
if [[ -n "${EXISTING_NETUIDS:-}" ]]; then
    log "Ensure max_regs_per_block=8 on existing subnets"
    for NETUID in "${NETUIDS[@]}"; do
        btcli_cmd sudo set --netuid "$NETUID" \
            --wallet-name "$ALICE_WALLET" --param max_regs_per_block --value 8 --no-prompt 2>&1 | tail -1
        ok "netuid $NETUID max_regs_per_block → 8"
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 2: Create 3 hotkeys per subnet + register validators
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 3: Stake TAO (ratio 3:2:1 per subnet)
# ═════════════════════════════════════════════════════════════════════════════
...
for i in 0 1 2; do
    ...
    for j in 0 1 2; do
        ...
        btcli_cmd stake add --wallet-name "$ALICE_WALLET" --hotkey "$HK" \
            --amount "$AMOUNT" --netuid "$NET" --no-prompt --unsafe 2>&1 | tail -2
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 4: Deploy contracts (DepositMailbox, SubnetClone, AlphaVault, ValidatorRegistry)
# ═════════════════════════════════════════════════════════════════════════════
...
# Resolve tokenIds (packed = netuid | (regBlock << 16)) — vault uses these as ERC1155 ids.
VAULT_IDS=()
...
# Push 3 validators per subnet with 50/30/20 BPS weights. Keys match STAKE_RATIOS so
# alpha ends up concentrated on the first hotkey, forcing _findBestMove to move it.
for i in 0 1 2; do
    NET="${NETUIDS[$i]}"
...
# Materialize subnet proxies (required before first deposit so clone's substrate coldkey exists).
for NET in "${NETUIDS[@]}"; do
...
# User's substrate coldkey — destination for alpha on withdraw.
WRAPPER_SUB_B32=$(h160_to_substrate_b32 "$WRAPPER_ADDR")
info "Wrapper substrate coldkey: $WRAPPER_SUB_B32"

# ═════════════════════════════════════════════════════════════════════════════
# PHASE 6: Alice transferStakes alpha → clones
# ═════════════════════════════════════════════════════════════════════════════

log "Phase 6: Transfer alpha → clone addresses"
echo "  (Contract will auto-rebalance across validators via moveStake)"
...
for i in 0 1 2; do
    ...
    # Alice moves alpha from her (hotkey,coldkey=Alice) slot → (hotkey,coldkey=Clone).
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 7: processDeposit → mint shares
# ═════════════════════════════════════════════════════════════════════════════
...
for i in 0 1 2; do
    ...
    TX_JSON=$(cast send "$VAULT_ADDR" \
        ...
        $EVM_FLAGS --gas-limit 1000000 --json 2>&1)
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 8: Verify deposit balances
# ═════════════════════════════════════════════════════════════════════════════

log "Phase 8: Verify deposits"

SHARES_CACHE=()  # indexed by vault position for use in Phase 9
DEPOSIT_TOTALS=()

for i in 0 1 2; do
    NET="${NETUIDS[$i]}"
    TID="${VAULT_IDS[$i]}"
    SHARES=$(cast call "$VAULT_ADDR" "balanceOf(address,uint256)(uint256)" "$WRAPPER_ADDR" "$TID" --rpc-url "$RPC_URL")
    TOTAL=$(cast call "$VAULT_ADDR" "totalStake(uint256)(uint256)" "$TID" --rpc-url "$RPC_URL")
    PRICE=$(cast call "$VAULT_ADDR" "sharePrice(uint256)(uint256)" "$TID" --rpc-url "$RPC_URL")
...
# ═════════════════════════════════════════════════════════════════════════════
# PHASE 9: Withdraw → verify alpha returned to user's substrate coldkey
# ═════════════════════════════════════════════════════════════════════════════

log "Phase 9: Withdraw all shares → verify alpha returned"

# Rebalance during withdraw's moveStake can lose ≤ 1 rao per hop; be generous.
TOLERANCE_RAO=10
...
for i in 0 1 2; do
    ...
    TX_JSON=$(cast send "$VAULT_ADDR" \
        ...
        --private-key "$WRAPPER_PK" --rpc-url "$RPC_URL" \
        $EVM_FLAGS --gas-limit 2000000 --json 2>&1)
    ...
    # Post-burn: user's share balance must be 0.
    POST_SHARES=$(cast call "$VAULT_ADDR" "balanceOf(address,uint256)(uint256)" "$WRAPPER_ADDR" "$TID" --rpc-url "$RPC_URL")
    [[ "$POST_SHARES" != "0" ]] && fail "netuid $NET: shares still $POST_SHARES after full withdraw"
    ok "netuid $NET: shares burned"

    # Alpha now sits at (one of the 3 hotkeys, WRAPPER_SUB_B32, netuid). Sum across all 3
    # to confirm ≥ deposited - tolerance landed.
    SUM=0
    for j in 0 1 2; do
        IDX=$((i * 3 + j))
        HK_B32="${ALL_HK_B32S[$IDX]}"
        BAL=$(cast call "$STAKING" "getStake(bytes32,bytes32,uint256)(uint256)" \
            "$HK_B32" "$WRAPPER_SUB_B32" "$NET" --rpc-url "$RPC_URL")
        SUM=$((SUM + BAL))
    done

    # deposited - tolerance <= sum (arithmetic in shell is bounded; use python for safety).
