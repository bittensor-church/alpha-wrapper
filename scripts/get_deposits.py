...
FIELDNAMES = [f.name for f in fields(DepositedEvent)]
...
def main() -> None:
    ...
    events = fetch_deposits(w3, args.vault_address, args.block_start, args.block_end)

    writer = make_csv_writer(sys.stdout, FIELDNAMES)
    for event in events:
        writer.writerow(asdict(event))

    print(f"Found {len(events)} Deposited events", file=sys.stderr)
