...
Event signature:
    ValidatorsUpdated(uint256 indexed netuid, uint8 count, uint256 timestamp)
"""
...
from common import (
    VALIDATORS_UPDATED_ARGS,
    VALIDATORS_UPDATED_SIG,
    assert_event_abi,
    get_web3_connection,
    load_abi,
    make_csv_writer,
...
def fetch_validator_updates(
    ...
    assert_event_abi(abi, "ValidatorsUpdated", VALIDATORS_UPDATED_ARGS)
    ...
    logs = w3.eth.get_logs({
        ...
        "toBlock": block_end,
        "address": address,
        "topics": [w3.keccak(text=VALIDATORS_UPDATED_SIG).hex()],
    })
