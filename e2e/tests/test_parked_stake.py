...
still counts it - the backing is exactly where its record expects - but every
stake operation naming the abandoned key is refused, so nobody can exit.
...
def test_stranded_holder_exits_after_anyone_claims_the_hotkey(env):
    ...
    # own record as sound, so the refusal below can only be the chain declining to
    # move alpha off a hotkey nobody owns.
