...
records that as the cause, so the loss is put on file and rides out its
recovery window before the position can be retired. The test restores the
threshold it found and then retires the swept position's shares, so sibling
suites sharing the session localnet see the subnet in a normal state.
"""
import pytest
