...
def test_array_items_parse_dynamic_array():
    assert chain._array_items("[0xaa, 0xbb]") == ["0xaa", "0xbb"]
    assert chain._array_items("[0xaa]") == ["0xaa"]
    assert chain._array_items("[]") == []
    assert chain._array_items("") == []
    # cast appends a bracketed scientific suffix to numeric elements.
    assert chain._array_items("[1000 [1e3], 2000 [2e3]]") == ["1000", "2000"]
