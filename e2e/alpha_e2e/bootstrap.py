...
def set_subnet_hyperparameter(netuid: int, parameter: str, value: int, *, check: bool = True) -> None:
    """Write a subnet hyperparameter via btcli sudo. Needs the admin freeze
    window disabled (build_environment does this) or the write can silently
    miss near an epoch boundary."""
    chain.btcli(
        ["sudo", "set", "--netuid", str(netuid), "--wallet-name", config.ALICE_WALLET,
         "--param", parameter, "--value", str(value), "--no-prompt"],
        check=check,
    )
...
def _create_subnets() -> List[int]:
    ...
    for netuid in netuids:
        ...
        set_subnet_hyperparameter(netuid, "max_regs_per_block", 8)
