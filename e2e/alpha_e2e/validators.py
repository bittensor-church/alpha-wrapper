...
def set_validators(
    ...
    *, deadline_secs: int = 3600, rpc_url: str = config.RPC_URL,
    ...
    # Gas estimation fails on the Bittensor EVM, so the limit is explicit.
    transaction = registry_contract.functions.updateValidators(
    ...
    ).build_transaction(
        {
            ...
            "gas": config.UPDATE_VALIDATORS_BASE_GAS
            + config.UPDATE_VALIDATORS_GAS_PER_HOTKEY * len(hotkeys),
