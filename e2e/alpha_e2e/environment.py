...
class Environment:
    ...
    def current_validators(self, netuid: int) -> List[str]:
        """The validator hotkeys the vault resolves for a subnet right now."""
        return chain.cast_call_array(
            self.vault_address, "getCurrentValidators(uint256)(bytes32[])", netuid,
        )

    def last_seen_hotkeys(self, token_id: int) -> List[str]:
        """The validator hotkeys the vault remembers for a token id."""
        return chain.cast_call_array(
            self.vault_address, "lastSeenHotkeys(uint256)(bytes32[])", token_id,
        )
    ...
    def hotkey_in_last_seen(self, token_id: int, hotkey_pubkey: str) -> bool:
        ...
        target = hotkey_pubkey.removeprefix("0x").lower()
        return any(
            hotkey.removeprefix("0x").lower() == target
            for hotkey in self.last_seen_hotkeys(token_id)
        )
