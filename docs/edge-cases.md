...
cleanup from the old position's own, so `unwrap` and `previewUnwrap` wait
until it completes. `wrap`, `rebalance` and `unwrapForTao` act on the
live generation only and stay frozen for the whole blackout.
