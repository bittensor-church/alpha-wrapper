...
same way; `scripts/plan_tao_exit.py` does exactly this. Quotes are taken one at
a time and earlier sales move the pool, so the dry run is the real check, and a
state change between it and inclusion can still call for one retry.
