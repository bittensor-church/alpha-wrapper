...
contract BackingRecordTest is AlphaVaultTestBase {
    ...
    function test_Swap_MovesActiveAndLeavesLogicalAlone() public {
        _depositAndWrap(alice, NETUID1, 30 ether);
        ...
        assertApproxEqAbs(vault.totalStake(TOKEN1), 30 ether, 0.01 ether, "backing whole across the swap");
    }

    /// @dev The record standing a hop ahead of the registry is the ordinary state after a swap,
    ///      and a second one must move it forward again rather than back to the key between.
    function test_RepeatedSwap_NeverAimsAtTheRetiredKey() public {
        uint256 netuid = 5;
        ...
        assertApproxEqAbs(vault.totalStake(tokenId), 10 ether, 0.01 ether, "backing whole across both swaps");
    }
    ...
    /// @dev Weights follow the validator the registry named, wherever a swap has since carried
    ...
    function test_SwapAfterWithdrawal_IsStillFollowed() public {
        ...
        assertTrue(vault.isBackingIntact(TOKEN1), "an ordinary swap after an exit is still followable");
        vault.rebalance(NETUID1);
    ...
    function test_SwapAfterTaoExit_StaysOperable() public {
        ...
        assertTrue(vault.isBackingIntact(TOKEN1), "the TAO rail leaves a record the swap cannot trip");
        vault.rebalance(NETUID1);
        assertApproxEqAbs(vault.totalStake(TOKEN1), backingAfterExit, 0.01 ether, "backing whole across the swap");
    }
