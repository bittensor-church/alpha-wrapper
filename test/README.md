...
| `BackingInvariantTest` | Public located/tracked backing is bounded by precompile balances across every touched hotkey plus the parking hotkey. A successful recovery leaves every recorded slot covered. | Validator changes, successor responses, moved stake, attempted deposits and exits, parked positions and their release. Refusals are allowed in this fault campaign; these bounds alone cannot establish conservation or liveness. |
