...
/// @dev Exercises the TAO-denominated min-stake floor (alpha * alpha_price >= 2e6). Each test
///      sets alpha_price below 1, where the alpha amount clears the old alpha threshold but the
///      tao-equivalent does not - the case that bricked the rails before the fix.
contract UnwrapMinStakeFloorTest is AlphaVaultTestBase {
    ...
    function test_Withdraw_SkipsSubFloorValidator_DeliversInFullFromNext() public {
        ...
        uint256 expectedAssets = _assetsFor(burnShares, 41e6, supply);

        vm.prank(alice);
        vault.withdraw(TOKEN1, burnShares, _toSubstrate(alice));

        uint256 received = _getStake(hotkey1, alice, NETUID1) + _getStake(hotkey2, alice, NETUID1);
        assertEq(received, expectedAssets, "skip-to-next delivers the request in full");
    ...
    function test_Rebalance_SkipsSubFloorMove_DoesNotRevert() public {
        _setValidators(NETUID1, _hotkeys(hotkey1, hotkey2), _weights(5000, 5000));
    ...
    function test_RotationSweep_DropsSubFloorTaoDust_DoesNotRevert() public {
    ...
    function test_Withdraw_RevertsAndKeepsShares_WhenEntireRequestBelowFloor() public {
        _setValidators(NETUID1, _hotkeys(hotkey1, hotkey2), _weights(5000, 5000));
        _simulateAlphaDepositHotkey(alice, NETUID1, 40e6, hotkey1);
        _processDepositHotkey(alice, NETUID1, hotkey1);

        _setAlphaPrice(NETUID1, PRICE_HALF);
        // Burning 5% targets ~2e6 of assets = 1e6 tao; every slice is sub-floor, so nothing is
        // transferable. The vault must revert rather than burn shares for a zero delivery.
        uint256 sharesBefore = vault.balanceOf(alice, TOKEN1);
        uint256 burnShares = sharesBefore * 5 / 100;

        vm.prank(alice);
        vm.expectRevert(AlphaVault.WithdrawTooSmall.selector);
        vault.withdraw(TOKEN1, burnShares, _toSubstrate(alice));

        assertEq(vault.balanceOf(alice, TOKEN1), sharesBefore, "shares preserved on revert");
    }
}
