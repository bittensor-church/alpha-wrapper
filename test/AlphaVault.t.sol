...
contract AlphaVaultTest is AlphaVaultTestBase {
    ...
    function test_Withdraw() public {
        ...
        vm.prank(alice);
        vault.withdraw(TOKEN1, shares, aliceSub, 0);
    ...
    function test_RevertWhen_WithdrawOnZero() public {
        ...
        vm.prank(alice);
        vm.expectRevert(AlphaVault.ZeroAmount.selector);
        vault.withdraw(TOKEN1, 0, aliceSub, 0);
    }
    ...
    function test_WithdrawPartialShares() public {
        ...
        // Withdraw half
        vm.prank(alice);
        vault.withdraw(TOKEN1, shares / 2, aliceSub, 0);
    ...
    function test_InterleavedDepositsWithdrawals() public {
        ...
        bytes32 aliceSub = _toSubstrate(alice);
        vm.prank(alice);
        vault.withdraw(TOKEN1, aliceShares, aliceSub, 0);
        assertEq(vault.balanceOf(alice, TOKEN1), 0);
        ...
        vm.prank(bob);
        vault.withdraw(TOKEN1, bobShares, bobSub, 0);
    ...
    function test_WithdrawEmitsRebalanced() public {
        ...
        vm.prank(alice);
        vault.withdraw(TOKEN2, shares / 2, _toSubstrate(alice), 0);
    }
    ...
    function test_RevertWhen_WithdrawInsufficientShares() public {
        ...
        vault.withdraw(tokenId, shares + 1, _toSubstrate(alice), 0);
    }
    ...
    function test_WihdrawFromDissolved() public {
        ...
        uint256 aliceBefore = alice.balance;
        vm.prank(alice);
        vault.withdraw(tokenId, shares, _toSubstrate(alice), 0);
        assertEq(alice.balance - aliceBefore, 50 ether);
    }
    ...
    function test_WithdrawFromDissolvedSubnetTwoHoldersProRata() public {
        ...
        uint256 aliceBefore = alice.balance;
        vm.prank(alice);
        vault.withdraw(tokenId, aliceShares, _toSubstrate(alice), 0);
        assertEq(alice.balance - aliceBefore, aliceExpected);

        uint256 bobBefore = bob.balance;
        vm.prank(bob);
        vault.withdraw(tokenId, bobShares, _toSubstrate(bob), 0);
    ...
    function test_PreviewWithdrawExcludesSubFloorOrphan() public {
        ...
        bytes32 cloneColdkey = _subnetColdkey(NETUID1);
        MockStaking(STAKING_PRECOMPILE).setStake(hotkey3, cloneColdkey, NETUID1, floor - 1);
    ...
    function test_PreviewWithdrawReturnsZeroWhenVaultDrained() public {
        ...
        MockStaking(STAKING_PRECOMPILE).setStake(hotkey1, cloneColdkey, NETUID1, 0);
        MockStaking(STAKING_PRECOMPILE).setStake(hotkey2, cloneColdkey, NETUID1, 0);
        MockStaking(STAKING_PRECOMPILE).setStake(hotkey3, cloneColdkey, NETUID1, 0);
    ...
    function test_LifecycleCaseBPruneRecycleWithNewSubnet() public {
        ...
        uint256 aliceBefore = alice.balance;
        vm.prank(alice);
        vault.withdraw(oldTokenId, aliceShares, _toSubstrate(alice), 0);
        assertEq(alice.balance - aliceBefore, 3 ether);

        uint256 bobShares = vault.balanceOf(bob, newTokenId);
        vm.prank(bob);
        vault.withdraw(newTokenId, bobShares, _toSubstrate(bob), 0);
        uint256 bobTotal = 0;
    ...
    function test_ProcessDepositCount1ChosenIsValidatorNoMoves() public {
        ...
        bytes32 cloneColdkey = _toSubstrate(vault.subnetClone(vault.currentTokenId(99)));
        assertEq(MockStaking(STAKING_PRECOMPILE).getStake(hotkey4, cloneColdkey, 99), 10 ether);
        assertEq(MockStaking(STAKING_PRECOMPILE).getStake(hotkey1, cloneColdkey, 99), 0);
        assertEq(MockStaking(STAKING_PRECOMPILE).getStake(hotkey2, cloneColdkey, 99), 0);
    }
    ...
    function test_ProcessDepositAcceptsExactlyMinStakeTaoFloorCount1() public {
        ...
        bytes32 cloneColdkey = _toSubstrate(vault.subnetClone(vault.currentTokenId(99)));
        assertEq(MockStaking(STAKING_PRECOMPILE).getStake(hotkey4, cloneColdkey, 99), 2e6);
    }
    ...
    function test_PartialWithdrawAccountsForEmissions() public {
        ...
        bytes32 aliceSub = _toSubstrate(alice);
        vm.prank(alice);
        vault.withdraw(TOKEN1, aliceShares / 2, aliceSub, 0);
    ...
    function testFuzz_WithdrawConservesAlpha(uint256 b1, uint256 b2, uint256 b3, uint256 burnPct) public {
        ...
        if (expectedAssets < minAmt) {
            ...
            vault.withdraw(TOKEN1, burnShares, aliceSub, 0);
            return;
        }
        ...
        // Replay the deterministic drain: walk [hk1, hk2, hk3] in order, taking min(balance,
        // remaining) from each slot, skipping any slice below the floor (price 1 => alpha < minAmt).
        uint256 remaining = expectedAssets;
        uint256 expectedDelivered;
        uint256[3] memory bals = [b1, b2, b3];
        for (uint256 i; i < 3; ++i) {
            uint256 take = remaining > bals[i] ? bals[i] : remaining;
            if (take >= minAmt) {
                expectedDelivered += take;
                remaining -= take;
            }
        }

        assertEq(vaultAfter + userReceived, b1 + b2 + b3, "withdraw conserves total alpha");
        assertEq(userReceived, expectedDelivered, "delivers exactly the floor-clearing slices");
    }
